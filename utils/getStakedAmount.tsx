...
const getStakedAmount = async () => {
  ...
  const realStartTime = new Date(Number(startTime) * 1000).toUTCString();
  const realEndDateInLocal = new Date(Number(endTime) * 1000);

  const realEndDateInUTC = {
    year: realEndDateInLocal.getUTCFullYear(),
    month: realEndDateInLocal.getUTCMonth(),
    date: realEndDateInLocal.getUTCDate(),
    time: realEndDateInLocal.getUTCHours(),
    min: realEndDateInLocal.getUTCMinutes(),
  };
